    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        // SỬA LẠI CHO API ĐỔI MẬT KHẨU
                        .requestMatchers(HttpMethod.GET, "/api/v1/test/**").authenticated() // <-- QUY TẮC CHO API TEST

                        // --- THÊM QUY TẮC BẢO VỆ ACTUATOR ---
                        // Yêu cầu phải có quyền 'MONITORING_VIEW' để truy cập actuator
                        .requestMatchers("/actuator/**").hasAuthority("MONITORING_VIEW")

                        // ... các quy tắc cũ cho /users, /test ...
                        .requestMatchers(HttpMethod.POST, "/api/v1/users/change-password").authenticated()
                        .requestMatchers("/api/v1/test/**").hasAnyAuthority("ROLE_USER", "USER_READ")
                        .anyRequest().denyAll())
